...
export default class extends Component {
    ...
    override async run(ctx: ComponentContext<ComponentType.Button>): Promise<any> {
        const insert = await ctx.database.query("INSERT INTO user_data (user_id, tokens) VALUES ($1, 69420) ON CONFLICT (user_id) DO UPDATE SET consent=true RETURNING *", [ctx.interaction.user.id]).catch(console.error)
